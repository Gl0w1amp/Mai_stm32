		...
		for (;;) {
			...
			if (start > 0u) {
				...
				serial_command_stream_len =
						(uint16_t)(serial_command_stream_len - start);
			}
			...
			if (serial_frame_checksum_valid(serial_command_stream, frame_len)) {
				if (serial_command_queue_push(serial_command_stream,
						frame_len) != 0u) {
					pushed_any = 1u;
				}
				memmove(serial_command_stream,
						serial_command_stream + frame_len,
						serial_command_stream_len - frame_len);
				serial_command_stream_len =
						(uint16_t)(serial_command_stream_len - frame_len);
				continue;
			}
